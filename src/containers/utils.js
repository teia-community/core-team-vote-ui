...
// Returns the votes
export async function getVotes(votesBigmap, network = NETWORK) {
    // Download all the votes from the token votes bigmap
    const votes = await getBigmapKeys(votesBigmap, {}, network);

    // Rearange the votes information in a dictionary
    const proposalVotes = votes ? {} : undefined;
    votes?.forEach(vote => {
        if (!(vote.key.nat in proposalVotes)) {
            proposalVotes[vote.key.nat] = {};
        }

        if (!(vote.value in proposalVotes[vote.key.nat])) {
            proposalVotes[vote.key.nat][vote.value] = 0;
        }

        proposalVotes[vote.key.nat][vote.value] += 1;
    });
